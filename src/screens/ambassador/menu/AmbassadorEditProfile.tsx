...
const AmbassadorEditProfile = (props: any) => {
            ...
            await AuthServices.prospect_complete_profile_fields().then((res: any) => {
                if (res.statusCode == 200) {
                    ...
                    if (mandatory_fields.length || optional_fields.length) {
                        let text = [...mandatory_fields].filter((t: any) => (t.field_type === 'text' || t.field_type === 'select_one'))
                        let select_one = [...mandatory_fields].filter((t: any) => (t.field_type === 'select_one'))
                        let select_one_opt = [...optional_fields].filter((t: any) => (t.field_type === 'select_one'))
                        let lang = optional_fields.filter((t: any) => (t.field_key === 'languages'))
                        let about1123 = mandatory_fields.filter((t: any) => (t.field_type === 'textarea'))
                        let customs = optional_fields.filter((t: any) => (t.field_type === 'custom_text'))
                        let date = optional_fields.filter((t: any) => (t.field_type === 'date'))
                        let data = optional_fields.filter((t: any) => (t.field_type === 'numberOnly' || t.field_type === 'date' || t.field_type === 'text'))
                        let email = mandatory_fields.filter((t: any) => (t.field_type === 'email'))
                        let profilePicture = mandatory_fields?.filter((t: any) => (t.field_key === 'profile_picture'))
                        ...
                        let o = select_one?.map((f: any) => {
                            d[f.field_key] = f?.field_value.id
                        })
                        ...
                        let k: any = {}
                        let g = select_one_opt?.map((f: any) => {
                        ...
                        setAbout(about1123[0].field_value)
                        setIsLoading(false)
                    }
                } else {
    ...
    const on_update = async () => {
        ...
        let send_data = {
            ...
            email: email,
            mandatory: {
                first_name: mvalue?.['first_name'].trim(),
                last_name: mvalue?.['last_name'].trim(),
                about_me: about,
                program: mvalue?.['program'],
                affiliation_with_institution: mvalue?.['affiliation_with_institution'],
                ...
                if (res.statusCode === 200) {
                    if (profilePic) {
                        ...
                        CometChat.updateCurrentUserDetails(user).then(
                            user => {
                                setBtnLoading(false)
                                props.navigation.navigate('AmbassPrfoile')
                            }, error => {
        ...
        return (
            <View key={item.field_key} >
                ...
                <MultiSelect
                    ...
                    labelField="label"
                    valueField="value"
                    placeholder={getSelectedItems(languageValue, item.field_placeholder)}
                    searchPlaceholder="Search..."
        ...
        return (
            <View key={item.field_key}>
                ...
                <TextInput
                    ...
                    className='flex flex-row mt-3 min-w-full h-[50px] px-3 text-left rounded-lg justify-start items-start text-textColor leading-4 bg-fieldGrayColor tracking-[0.44px] font-InterRegular font-normal'
                    value={about}
                    onChangeText={(e: any) => {
                        const cleanedValue: any = e.replace(/^\s+|\s+$|\s+(?=\s)/g, " ");
                        if (cleanedValue) {
    ...
    return (
        <View className='flex flex-1 bg-bgGrayColor'>
            ...
            <View className={`flex flex-row items-center px-2 py-5`} style={{ backgroundColor: primaryColor }}>
                <TouchableOpacity onPress={() => { props.navigation.navigate('AmbassPrfoile') }}>
...
export default AmbassadorEditProfile