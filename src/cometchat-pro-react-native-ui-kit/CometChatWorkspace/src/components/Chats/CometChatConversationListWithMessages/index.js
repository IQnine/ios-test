...
import AmbassadorHeader from '../../../../../../screens/ambassador/header/AmbassadorHeader';
...
class CometChatConversationListWithMessages extends React.Component {
  ...
  componentDidMount() {
    ...
    try {
      ...
      let { item, loggedInUser, type, admin, college_name, receiver } = params
      if (item) {
        const users = [loggedInUser.uid, item.cometchat_uid];
        conversationId = users.sort().join('_user_');
        let i = {
          blockedByMe: loggedInUser.blockedByMe,
          conversationId: conversationId,
          deactivatedAt: loggedInUser.deactivatedAt,
          hasBlockedMe: loggedInUser.hasBlockedMe,
          lastActiveAt: null,
          name: item.user_detail.first_name,
          role: loggedInUser.role,
          status: item.last_active,
          uid: item.cometchat_uid,
          avatar: item.user_detail.profile_image
        }
        this.setState({ loggedInUser: loggedInUser })
        this.itemClicked(i, type, navigation_type = 'AMBASSADOR')
      }
      else if (loggedInUser && type && admin && college_name) {
        // let a_id = cometchat_feedback_uid();
        const users = [loggedInUser.uid, loggedInUser.feedback_uid];
        conversationId = users.sort().join('_user_');
        let i = {
          blockedByMe: loggedInUser.blockedByMe,
          conversationId: conversationId,
          deactivatedAt: loggedInUser.deactivatedAt,
          hasBlockedMe: loggedInUser.hasBlockedMe,
          lastActiveAt: loggedInUser.lastActiveAt,
          name: loggedInUser.name,
          role: loggedInUser.role,
          status: loggedInUser.status,
          uid: loggedInUser.feedback_uid
        }
        this.setState({ loggedInUser: loggedInUser })
        this.itemClicked(i, type, navigation_type = 'ADMIN_CHAT')
      }
      else if (loggedInUser && type) {
        const users = [loggedInUser.uid, SUPER_ADMIN_ID.staging];
        conversationId = users.sort().join('_user_');
        let i = {
          blockedByMe: loggedInUser.blockedByMe,
          conversationId: conversationId,
          ...
          role: loggedInUser.role,
          status: loggedInUser.status,
          uid: SUPER_ADMIN_ID.staging
        }
        this.setState({ loggedInUser: loggedInUser })
        this.itemClicked(i, type, navigation_type = 'TECH_SUPPORT')
      }
  ...
  getuserDetails = async () => {
    let t = await Utils.getData('user_details');
    this.user_detail = t;
  }
   ...
   */
  itemClicked = (item, type, navigation_type) => {
  ...
  groupUpdated = (message, key, group, options) => {
    switch (key) {
      ...
      case enums.GROUP_MEMBER_KICKED: {
        if (options.user.uid === this.state.loggedInUser.uid) {
    ...
    this.setState({ callMessage: call }, () => {
      CometChat.CometChatHelper.getConversationFromMessage(call)
        .then(conversation => {
          ...
          this.itemClicked(conversationWith, conversationType, navigation_type = '');
        })
        .catch(err => {
          logger(err)
        })
  ...
  membersAdded = (members) => {
    ...
    members.forEach((eachMember) => {
      const message = `${this.state.loggedInUser.name} added ${eachMember.name}`;
  ...
  memberUnbanned = (members) => {
    const messageList = [];
    members.forEach((eachMember) => {
      const message = `${this.state.loggedInUser.name} unbanned ${eachMember.name}`;
      const sentAt = (new Date() / 1000) | 0;
      const messageObj = {
        category: 'action',
        message,
  ...
  memberScopeChanged = (members) => {
    ...
    members.forEach((eachMember) => {
      const message = `${this.state.loggedInUser.name} made ${eachMember.name} ${eachMember.scope}`;
      const sentAt = (new Date() / 1000) | 0;
      const messageObj = {
        category: 'action',
        message,
  ...
  render() {
    ...
    return (
      <CometChatContextProvider ref={(el) => (this.contextProviderRef = el)}>
        {
          this.user_detail?.role_id == 5 ?
            <Header navigation={this.props.navigation} collegeInfo={this.props.collegeInfo} />
            :
            <AmbassadorHeader navigation={this.props.navigation} />
        }
        {
...
export default connect(mapStateToProps)(CometChatConversationListWithMessages);
